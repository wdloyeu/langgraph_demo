...
async def create_graph():
    ...
    # graph = builder.compile(interrupt_before=['tools']) # langgraph dev自动运行时不需要加checkpoint
    # 手动运行，需要加checkpoint
    # 检查点让状态图可以持久化其状态
    # 这是整个状态图的完整内存
    memory = MemorySaver()
...
async def run_graph():
    graph = await create_graph() #配置参数，包含乘客ID和线程ID
    config = {
        "configurable":{
            # 检查点由session_id访问
            "thread_id": 'zsi2311',
    ...
    def print_message(event, result):
        ...
        if messages:
            if isinstance(messages, list):
                ...
                if message and message.__class__.__name__ == 'AIMessage':
                    if message.content:
                        print(result)
    ...
    async def execute_graph(user_input: str) -> str:
        ...
        if current_state.next: # 出现了工作流的中断
            ...
            result = f"AI助手 马上根据你要求，执行{tool_name}工具。您是否批准继续执行？输入'y'继续：否则，请说明您理由。\n"
        return result
