...
class BasicToolsNode:
    ...
    async def __call__(
        ...
        review_tool_call = next(
            (
                tool_call
                for tool_call in message.tool_calls
                if tool_call.get("name") in review_tool_names
            ),
            None
        )
        ...
        if review_tool_call is not None:
            ...
            tool_name = review_tool_call["name"]
            ...
            # 人工批准：继续执行原来的全部 tool_calls
            if answer == "y":
                pass

            # 人工拒绝：必须给 AIMessage 中的每一个 tool_call
            # 返回一个对应的 ToolMessage
            else:
                outputs = []
